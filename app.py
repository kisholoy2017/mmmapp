...
if tab_selection == "📤 Data Upload":
    ...
    if promo_file:
        try:
            ...
            # Don't clean the promotion column, keep it as-is (string or numeric)
            # Only clean if there are additional numeric columns
        ...
        if st.session_state.kpi_data is None:
            ...
            with st.spinner("Combining data..."):
                try:
                    ...
                    # Final cleaning of all numeric columns (except date)
    ...
    if not st.session_state.data_uploaded:
        ...
        with validation_col1:
            ...
            if date_range_months >= 24:
                ...
                st.warning(f"⚠️ Limited data: {date_range_months:.1f} months (<24 months recommended)")
                st.caption("Note: Models work best with 24+ months, but you can still proceed")
        ...
        # Get numeric columns only for styling
...
# TAB 3: Marketing Mix Modeling
    ...
    if not st.session_state.data_uploaded:
        ...
        if date_range_months < 24:
            ...
            st.info("📊 MMM models work best with 24+ months of data, but you can proceed with available data. Results may be less reliable.")
        else:
        ...
        if st.button("🚀 Run Marketing Mix Model", type="primary", use_container_width=True):
            with st.spinner("Training Marketing Mix Model... This may take a few minutes."):
                try:
                    ...
                    # Ensure control columns are numeric where needed
                    for ctrl_col in all_control_cols:
                        if ctrl_col in train_df.columns:
                            ...
                            if train_df[ctrl_col].dtype == 'object':
                                # This should already be handled for dummies, but check anyway
                                try:
